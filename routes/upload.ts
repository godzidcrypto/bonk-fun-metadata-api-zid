...
import { ImageUploadSchema } from "../validation/image_validation";
import { arktypeValidator } from "@hono/arktype-validator";
import { TokenMetadataSchema } from "../validation/metadata_validation";
...
uploadRouter.post(
    ...
    arktypeValidator('form', ImageUploadSchema, (result, c) => {
        if (!result.success) {
            ...
            return c.text(`You have provided an incorrect image:\n${result.errors.summary}`, 400);
        }
    }),
    async (c) => {
...
uploadRouter.post(
    ...
    arktypeValidator('json', TokenMetadataSchema, (result, c) => {
        if (!result.success) {
            console.error('Validation Problems:', result.errors.summary);
            return c.text(`You have provided incorrect token metadata:\n${result.errors.summary}`, 400);
        }
    }),
    async (c) => {
        ...
        try {
            // Create file
            const metaFile = new File([JSON.stringify(validatedMeta)], `metadata_${Date.now()}.json`, { type: "application/json" });
