...
import { contestRegistrationSchema, userSchema } from '../db/schema';
  ...
  async (c) => {
    try {
      let { contestId, twitter, profileImageUrl } = c.req.valid('json') as {
        contestId: string; twitter: string; profileImageUrl: string
      };
      ...
      if (!twitterValid) {
        return c.text('Invalid twitter handle. Use 1-15 chars: letters, numbers, underscore.', 400);
      }

      // Optional: verify pubkey is a valid Solana address
      try { new PublicKey(pubkey) } catch { return c.text('Invalid wallet in JWT', 400) }
      ...
      if (sol < MIN_SOL_REQUIRED) {
        return c.text(`Insufficient SOL balance (need ${MIN_SOL_REQUIRED} SOL)`, 400);
      }
      ...
      const existing = await db.query.contestRegistrationSchema.findFirst({
        where: (t, { eq, and }) => and(eq(t.contestId, contestId), eq(t.userPubkey, pubkey))
      });
...
contestRouter.get('/leaderboard/:contestId', async (c) => {
  try {
    ...
    const rows = await db
      ...
      .where(and(eq(contestRegistrationSchema.contestId, contestId), eq(contestRegistrationSchema.approved, true)))
...
contestRouter.post(
  ...
  async (c) => {
    if (!isAdminAuthorized(c)) return c.text('Unauthorized', 401);
    try {
      const { id, approved } = c.req.valid('json') as { id: number; approved: boolean };
      await db
        .update(contestRegistrationSchema)
        .set({ approved })
