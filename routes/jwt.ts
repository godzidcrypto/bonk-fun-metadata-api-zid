...
if (!metadataEnv.JWT_SALT) {
    console.warn("[JWT] ⚠️ No cryptographic salt was provided, the default one will be used which may be insecure.");
}
...
function getNonceForPublicKey(publicKey: PublicKey | string) {
    publicKey = publicKey instanceof PublicKey ? publicKey.toBase58() : publicKey.toString();
 ...
 */
function verifyWalletSignature(publicKey: PublicKey | string, signature: string) {
