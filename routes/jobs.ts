...
import { jobApplicationSchema, jobSchema } from "../db/schema.js";
...
jobsRouter.get("/", arktypeValidator("query", ListQuery), async (c) => {
  try {
    const { q, company, seniority, location, salaryMin, salaryMax } =
    ...
    const pageSize = Math.min(
      50,
      Number.parseInt(c.req.query("pageSize") ?? "20", 10)
    );
    ...
    if (q) {
      ...
      whereClauses.push(
        sql`lower(${jobSchema.title}) like ${likeExpr} or lower(${jobSchema.company}) like ${likeExpr}` as any
      );
    }
    ...
    if (location)
      whereClauses.push(like(jobSchema.location, `%${location}%`) as any);
    const sMin = salaryMin ? Number.parseInt(salaryMin, 10) : undefined;
    const sMax = salaryMax ? Number.parseInt(salaryMax, 10) : undefined;
    if (Number.isFinite(sMin as number))
      whereClauses.push(sql`${jobSchema.salaryMin} >= ${sMin}` as any);
    if (Number.isFinite(sMax as number))
      whereClauses.push(sql`${jobSchema.salaryMax} <= ${sMax}` as any);
...
jobsRouter.post(
  "/apply",
  arktypeValidator("json", ApplySchema, (result, c) => {
    if (!result.success)
      return c.text(`Invalid data: ${result.errors.summary}`, 400);
  }),
  async (c) => {
    try {
      const { jobId, email, note } = c.req.valid("json") as {
        jobId: number;
        email: string;
        note?: string;
      };

      const job = await db.query.jobSchema.findFirst({
      ...
      const existing = await db.query.jobApplicationSchema.findFirst({
        where: (t, { eq, and }) =>
          and(eq(t.jobId, jobId), eq(t.email, email.toLowerCase())),
      });
      if (existing)
        return c.text(
...
jobsRouter.post(
  "/admin/create",
  arktypeValidator("json", CreateJobSchema, (r, c) => {
  ...
  async (c) => {
    ...
    try {
      ...
      const sanitize = (s?: string, max = 256) =>
...
jobsRouter.post(
  "/admin/update",
  arktypeValidator("json", UpdateJobSchema, (r, c) => {
  ...
  async (c) => {
    ...
    try {
      ...
      if (typeof update.title !== "undefined")
        update.title = sanitize(update.title, 160);
      if (typeof update.company !== "undefined")
        update.company = sanitize(update.company, 80);
      if (typeof update.logo !== "undefined")
        update.logo = sanitize(update.logo, 512);
      if (typeof update.location !== "undefined")
        update.location = sanitize(update.location, 120);
      if (typeof update.employmentType !== "undefined")
        update.employmentType = sanitize(update.employmentType, 40);
      if (typeof update.seniority !== "undefined")
        update.seniority = sanitize(update.seniority, 40);
      if (typeof update.salary !== "undefined")
        update.salary = sanitize(update.salary, 120);
      if (typeof update.description !== "undefined")
        update.description = sanitize(update.description, 5000);
...
jobsRouter.post("/admin/applications/delete", async (c) => {
  ...
  try {
    ...
    await db
      .delete(jobApplicationSchema)
      .where(eq(jobApplicationSchema.id, payload.id));
    return c.json({ status: "ok" });
  } catch (e) {
...
jobsRouter.post("/admin/delete", async (c) => {
  ...
  try {
    ...
    await db
      .delete(jobApplicationSchema)
      .where(eq(jobApplicationSchema.jobId, payload.id));
