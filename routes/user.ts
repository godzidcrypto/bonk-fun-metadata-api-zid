...
import { pubkeyValidator } from "../validation/pubkey_validation.js";
import { UserAssignSchema } from "../validation/user_assign_validation.js";
...
userRouter.post(
  ...
  arktypeValidator("json", UserAssignSchema, (result, c) => {
    if (!result.success) {
      ...
      return c.text(
        `You have provided incorrect user profile data:\n${result.errors.summary}`,
        400
      );
    }
  }),
...
userRouter.get(
  ...
  arktypeValidator("param", type({ pubkey: pubkeyValidator }), (result, c) => {
    if (!result.success) {
      console.error("Validation Problems:", result.errors.summary);
      return c.text(
        `You have provided incorrect public key data:\n${result.errors.summary}`,
        400
      );
    }
  }),
  ...
  async (c) => {
    ...
    try {
      ...
      const tokenRes = await fetch(
        `${KICK_OAUTH_URL.replace(/\/$/, "")}/oauth/token`,
        {
          method: "POST",
      ...
      console.log(
        "Kick User API URL:",
        `${KICK_API_URL.replace(/\/$/, "")}/public/v1/users`
      );
      ...
      const meJson = (await meRes.json()) as {
        data?: Array<{ name?: string; user_id?: number; email?: string }>;
      };
...
userRouter.delete("/connections/kick", requireWalletJWT, async (c) => {
  ...
  try {
    ...
    await db
      .update(userSchema)
      .set({ kickAccessToken: null, kickUsername: null })
      .where(eq(userSchema.pubkey, pubkey));
